...
async def produce(msg: dict):
    ...
    try:
        ...
        obj = LegalDataObject(msg)

        msg = obj.standardData(sensitiveDataObj.identify_sensitive_data(msg))
...
async def consume(msg_test):
    ...
    consumer = AIOKafkaConsumer(
        'lawsuit_classified',
        bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
        group_id="classifier-test-group",
    ...
    try:
        # Pega a ultima mensagem enviada para poder comparar

        topic_partition = TopicPartition('lawsuit_classified', 0) 
